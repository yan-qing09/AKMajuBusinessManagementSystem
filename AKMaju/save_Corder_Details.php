...
$sql5 = "SELECT * FROM tb_construction_order WHERE CO_id = '$co_id'";
...
$updateQueries = [
    "UPDATE tb_construction_order SET CO_quotationStatus = ? WHERE CO_id = ?",
];

$statusValues = [$Qstatus];
$statusColumns = ['CO_quotationStatus'];
...
$sqlEKawasan = "UPDATE tb_construction_order SET EK_negeri = ?, EK_daerah = ?, EK_jarak = ?, EK_tambahan = ? WHERE CO_id = ?";
$stmtEKawasan = $con->prepare($sqlEKawasan);
$stmtEKawasan->bind_param("sssis", $EKnegeri, $EKdaerah, $EKkawasan, $EKtambahan, $newOrderId);
...
$sqlAkawasan = "UPDATE tb_construction_order SET AK_negeri = ?, AK_daerah = ?, AK_jarak = ?, AK_tambahan = ? WHERE CO_id = ?";
$stmtAkawasan = $con->prepare($sqlAkawasan);
$stmtAkawasan->bind_param("sssis", $AKnegeri, $AKdaerah, $AKkawasan, $AKtambahan, $newOrderId);
...
$redirectURL = 'EditCEOrdermaterial.php?id=' . $rows['U_id'] . '&co_id=' . $row6['CO_id'];
